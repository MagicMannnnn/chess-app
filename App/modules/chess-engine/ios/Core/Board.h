...
private:
    ...
    struct MoveRecord {
        ...
        int halfmoveClock;
        uint64_t previousPositionHash;
        std::array<Square, 2> previousKingSquares;
    };
    ...
    void generatePieceMovesInto(Square from, Color color, std::vector<Move>& moves) const;
    bool wouldBeInCheck(const Move& move, Color color) const;
    void makeMoveUnchecked(const Move& move);
    void unmakeMoveUnchecked();
