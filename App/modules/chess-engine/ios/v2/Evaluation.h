...
struct CastlingRights {
    ...
    CastlingRights() : whiteKingSide(false), whiteQueenSide(false),
                       blackKingSide(false), blackQueenSide(false) {}
};

// Material-first evaluation strategy: 
// Material is the primary score, positional factors are minimal bonuses
class EvaluatorV2 {
public:
    ...
    // Material is multiplied by 10 to ensure it dominates all other factors
    static int evaluate(
        const Board& board, 
        Color aiColor, 
    ...
    // Piece development evaluation (small bonuses/penalties)
    ...
    // Piece coordination - small bonuses for active, coordinated play
